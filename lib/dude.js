class Dude{
    ...
    constructor(x,y,brain){
        ...
        this.brain = brain ||new NeuralNetwork(6);
        this.brain.addLayer(8,'dense','relu')
        this.brain.addLayer(8,'dense','relu')
        ...
        6:(num)=> {return this.shoot(num)},
        }
        
        
        this.mainbulletlistref =[];
       /* this.seen = {
            front:false,
            left:false,
            right:false  
    ...
    init(){
       ...
       /* this.body.vision.front = new Div(-10,-90,'white',40,90,true)
        this.body.vision.left = new Div(-40,-20,'white',40,40,true)
        this.body.vision.right = new Div(18,-20,'white',40,40,true)*/
        this.body.head.add(this.body.eye1);
        this.body.head.add(this.body.eye2);
      /*  this.body.head.add(this.body.vision.front);
        this.body.head.add(this.body.vision.left);
        this.body.head.add(this.body.vision.right);*/
        document.body.appendChild(this.body.head.shape);
     /*   
        this.vision = {
        front: new Rect(this.p.x-21,this.p.y-99,40,90),
        left: new Rect(this.p.x-51,this.p.y-29,40,40),
        right: new Rect(this.p.x+7,this.p.y-29,40,40)
        };*/
      /*  this.vision.front.config.rotation= 'custom';
        this.vision.left.config.rotation= 'custom';
        this.vision.right.config.rotation= 'custom';*/
        this.initSensors()
    }
    initSensors(){
        let front = new Sensor(this,new Rect(this.p.x-21,this.p.y-99,40,90),'front');
        let left = new Sensor(this,new Rect(this.p.x-51,this.p.y-29,40,40),'left');
        let right = new Sensor(this,new Rect(this.p.x+7,this.p.y-29,40,40),'right');
        this.sensors.push(left,front,right)
    }

    checkBounds(){
        if(this.p.y>this.canvasSize.y-20){this.p.y=this.canvasSize.y-20;  this.life-=this.penalty;}
        if(this.p.x>this.canvasSize.x-20){this.p.x=this.canvasSize.x-20;  this.life-=this.penalty;}
        if(this.p.y<0){this.p.y=0;  this.life-=this.penalty;}
        if(this.p.x<0){this.p.x=0;  this.life-=this.penalty;}
        if(this.points<0){
            this.points = 0; 
        }
    }
    ...
    move(arr){
        ...
        arr.forEach((x,i)=>{
            if(i === 6){
               if( x>0.4){
               this.mainbulletlistref.push( this.actions[i]())
            }
            }else{
                if(x>0.1){
   ...
   goBackward(num){
       return this.fw.copy().mult(-.1).mult(num)
   }
   strafeLeft(num){
      return this.fw.copy().perp().mult(.5).mult(num)
   }
   strafeRight(num){
      return this.fw.copy().perp().mult(-.5).mult(num)
   }
    ...
    updateSensors(){
        if(this.sensors.length){
            this.sensors.forEach(s=>{
                s.moveTo(this.p).rotateTo(this.fw.getAngle(),this.p.x-2,this.p.y-2)
            })
        }
    }
    remove(){
