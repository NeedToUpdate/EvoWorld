class nnOverlay {
  constructor(nn) {
    ...
    this.rect.shape.style.position = "fixed";
      ...
      arr.forEach((x, j) => {
        ...
        if (i !== this.num_of_layers - 1) {
          for (let k = 0; k < this.layers[i + 1].length; k++) {
            ...
            let r = new Div(this.x + colRes * i + 10, this.y + rowRes * (j + 1), this.getCol(this.nn.layers[i].weight_array[0][0].values[k][j]), w, 0, true, ang * (180 / Math.PI), true);

            r.shape.style.position = "fixed";
            this.lines[i][j].push(r);
          }
        }
        let c = new Div(this.x + colRes * i + 10, this.y + rowRes * (j + 1), "blue", Math.sqrt(rowRes));

        c.shape.style.position = "fixed";
