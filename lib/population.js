class Population{
     ...
     newGen(){
         this.pointaverage = (this.saved.reduce((tot,a)=>tot+=a.getFitness(),0))/this.saved.length;
     ...
     cloneAndMutateWithNewPops(){
         ...
         for(let i=0; i<this.popNum-2; i++){
              let newdude = this.generator();
                 newdude.brain =  this.best.brain.copy();
              newdude.init();
                  newdude.brain.mutate(.001,.5);
                  newdude.brain.mutate(.4,0.001);
                  newdude.brain.mutate(this.config.mutRate,this.config.mutAmount);
     ...
     chooseFittestAndMix(){
         ...
         //console.log('points: ' + this.saveddudes.map(x=>x.getFitness()))
          // console.log('sum: ' + sum)
         for(let i = 0; i<2; i++){
             let sum= this.saved.reduce((tot,a)=>{ return tot+=a.getFitness()},0);
