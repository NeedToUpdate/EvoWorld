let offsets = {
  ...
  right_side: { p: new Vector(UNIT * 0.5, UNIT * 1.5), r: -90 },
  left_side: { p: new Vector(UNIT * -0.5, UNIT * 1.5), r: 90 },
  left_butt: { p: new Vector(UNIT * 0.5, UNIT * 1.5), r: 90 },
  right_butt: { p: new Vector(UNIT * -0.5, UNIT * 1.5), r: -90 },
};
...
class Eye extends Appendage {
  constructor(x, y, r, location) {
    ...
    this.name = "eye";
    this.hitbox = new Hitbox(x + UNIT / 2, y + UNIT * 2, UNIT, UNIT * 10);
    this.hasInput = true;
  }
  use(arrayOfPoints) {
    let found = [];
